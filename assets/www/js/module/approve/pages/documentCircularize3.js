...
function getContentData(data) {
    ...
    firstAssigneeList = data.firstAssigneeList;
...
function initContentInterface(){
    ...
    if(instance.isOperator && (instance.currentStep == "发起流程" || instance.currentStep == "科室审批" || instance.currentStep == "行政办确认")) {
        ...
        checkList.forEach(function (value) {
            if (value.from == undefined){
                ...
                var newPerson =
                    "<li>" +
                    "<a class='name'>"+value.name+"("+value.from+"转)</a>" +
                    "<a class='del' title='删除' data-id='"+value.id+"' onclick='deletePerson(this)'>㊀</a>" +
                    "</li>";
            }
    ...
    if(instance.isOperator && (instance.currentStep == "发起流程" || instance.currentStep == "科室审批" ||instance.currentStep == "行政办确认" || (instance.currentStep == "传阅人审批"&&firstAssigneeList.indexOf(user.id) >-1 ))){
    ...
    initBranchApproveControl(instance.currentStep == "传阅人审批",timeAdviceSection);//分管领导审批信息显示控制
}

$(document).ready(
    function init() {
        initCommon("documentCircularize3");
